...
I (Olivier Lance) have forked has_machine_tags to add specific features I need for one of my projects:
* Add a _no_duplicates_ option to has_machine_tags to avoid duplicate tags on model *(DONE)*
* Add a _match_all_ option to tagged_with to be able to find models tagged with all given tags (regular AND machine tags) *(DONE)*
* Add a _comparison_match_ option to allow queries such as: *(DONE)*
    Model.tagged_with "geo:lat>=0, geo:lat<=90", { :match_all => true, :comparison_match => true } # in northern hemisphere
