...
* Add a _no_duplicates_ option to ```has_machine_tags``` to avoid duplicate tags on model
* Add a _match_all_ option to ```tagged_with``` to be able to find models tagged with all given tags (regular AND machine tags)
* Add a _numeric_match_ option to allow queries such as ```Model.tagged_with "geo:lat>=0, geo:lat<=90" # in northern hemisphere```
...
* <s>Add a match_all option to tagged_with().</s>
