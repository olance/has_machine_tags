...
* Add a *no_duplicates* option to `has_machine_tags` to avoid duplicate tags on model
* Add a *match_all* option to `tagged_with` to be able to find models tagged with all given tags (regular AND machine tags)
* Add a *numeric_match* option to allow queries such as `Model.tagged_with "geo:lat>=0, geo:lat<=90" # in northern hemisphere`
